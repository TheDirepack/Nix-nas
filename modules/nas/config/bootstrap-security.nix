...
let
  ...
  bootstrapAuthentikPassword = "nas-admin-first-boot";
  ...
  bootstrapSecrets = pkgs.writeShellScript "nas-bootstrap-kdbx-secrets" ''
    ...
    # This human bootstrap credential is intentionally fixed and documented so
    # a fresh appliance can always be entered through the web setup flow. It is
    # scoped to the disposable bootstrap Authentik authority and is retired
    # with that authority after the operator creates permanent credentials.
    ensure_entry authentik-bootstrap-password ${lib.escapeShellArg bootstrapAuthentikPassword}
    ...
    [[ "$bootstrap_password" == ${lib.escapeShellArg bootstrapAuthentikPassword} ]] || exit 74
  ...
  runtimeSelector = pkgs.writeShellScript "nas-bootstrap-runtime-select-secure" ''
    ...
    bootstrap_root=${lib.escapeShellArg bootstrapRuntimeRoot}
    if [[ -e /var/lib/nas-setup/operational-runtime-select || -e /var/lib/nas-setup/state.json ]]; then
    ...
    runtime_environment=${lib.escapeShellArg authentikRuntimeEnvironmentFile}
    runtime_api_token=${lib.escapeShellArg authentikRuntimeApiTokenFile}

    if [[ "$target" == "$bootstrap_root" ]]; then
      for source in "$environment" "$api_token"; do
        if [[ -L "$source" || ! -f "$source" ]]; then
          echo "Bootstrap credential source must be a regular non-symlink file: $source" >&2
          exit 78
        fi
      done
      for destination in "$runtime_environment" "$runtime_api_token"; do
      ...
      ${pkgs.coreutils}/bin/install -m 0640 -o root -g authentik "$environment" "$runtime_environment"
      ${pkgs.coreutils}/bin/install -m 0400 -o root -g root "$api_token" "$runtime_api_token"
    else
      for spec in \
        "$runtime_environment:$environment" \
        "$runtime_api_token:$api_token"; do
        destination="''${spec%%:*}"
        source="''${spec#*:}"
        if [[ -e "$destination" && ! -L "$destination" ]]; then
...
in
{
  ...
  systemd.services.nas-bootstrap-authentik-secrets = {
    ...
    serviceConfig = {
      ...
      ExecStart = lib.mkOverride 40 bootstrapSecrets;
      UMask = "0077";
    };
  };
  ...
  systemd.services.nas-first-run-api.environment.NAS_FIRST_START_JOB =
    "${nasFirstStart}/bin/nas-first-start-job";
}
