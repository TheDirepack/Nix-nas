...
let
  ...
  nasZfsCreateEncryptedDataset = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      actor_groups=" $(id -Gn) "
      if [[ "$actor" == root ]]; then
        [[ "''${NAS_SETUP_ALLOW_ROOT:-}" == 1 && "''${NAS_OPERATION_COORDINATION_TOKEN:-}" =~ ^[0-9a-f]{32}$ ]] || {
          echo "Root may create the managed encryption root only through the authenticated, coordinated setup service." >&2
          exit 1
        }
