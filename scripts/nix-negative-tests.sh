...
cases=(
  ...
  'tests/nixos/invalid/replication-leading-dash.nix|nas.zfsReplication.enable requires a non-empty destination'
...
for row in "${cases[@]}"; do
  ...
  log="$(mktemp)"
  trap 'rm -f -- "$log"' EXIT
  if nix eval --impure --raw --file tests/nixos/negative-eval.nix \
      --argstr root "$ROOT" --argstr fixture "$fixture" >"$log" 2>&1; then
    cat "$log" >&2
