# Unified Managed Services — Current Implementation Status (as of 2.2.0-alpha.29)
...
The plan describes a 28-phase, zero-extra-daemon system for Podman/Compose/VM + Caddy + firewalld + Authentik + portal. The current branch is at **~75%** of that plan — the registry → effective → portal → gate → Caddy pipeline is fail-closed and hardened, and is the **only** path for service definitions. All 13 built-ins (identity, cockpit, aiApi, aiRuntime, aiWorkspace, aiDownloader, syncthing, vaultwarden, victoriametrics, grafana, alerts, notifications, ups) are defined as `ownership=system` entries in `service-registry.nix` v2 (`schemaVersion:2`, `services` with `runtime`/`endpoints`/`auth`/`portal`). Runtime adapters for Podman, Compose, VM, firewalld, and Authentik are callable by the managed-service CLI, and the Cockpit API exposes the unified registry.
...
| Service registry merge                 | 🟢 Done — now the only system | `service-registry.nix` now emits `serviceRegistryV2` (`schemaVersion:2`, `services` with `runtime`/`endpoints`/`auth`/`portal`, `ownership=system` for all 13 built-ins). `effective_registry()` handles both v1 and v2 built-ins and merges with runtime `services.json` into a single v2 effective model (`/run/nas-control/effective-endpoints.json`, `generation`). `schemas/service-registry.schema.json` is now `const:2` with `services`/`generation`. No consumer asks v1 vs v2. |
...
| firewalld policy generation            | 🟢 Adapter present | `services/nas_service_firewall.py` implements `plan`/`apply`/`remove` → `firewall-cmd --permanent` + `ipaddress` CIDR validation, with protocol-preserving forward rules and cleanup. |
| Authentik application/OIDC integration | 🟢 Adapter present | `services/nas_service_authentik.py` implements `plan`/`apply` for `forward-auth` vs `oidc` per endpoint (stable Authentik references, no membership copy); OIDC apply validates the referenced provider. |
...
- Automatic file-watch reconciliation still refreshes projections but does not start or stop external runtimes. The explicit `nas-managed-service create/update/delete/start/stop/restart/adopt/import` commands call the runtime, firewall, and Authentik adapters transactionally; end-to-end qualification of each external provider and Authentik resource provisioning remains outstanding.
...
- UI is still source-only (not install-ready) until the complete VM, installer, and hardware qualification gates pass.
