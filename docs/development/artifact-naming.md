...
source archive filename:        Nix OS NAS 2.2.13 source.zip
```
...
```text
nixos-nas-2.2.0-alpha.13-source-only-unverified/
```
...
```text
Nix OS NAS 2.2.13 source.zip
Nix OS NAS 2.2.13 source.zip.sha256
...
For the alpha line, increment the alpha revision (`2.2.0-alpha.12` -> `2.2.0-alpha.13`) for the next code-bearing revision. Keep `CHANGELOG.md`, Cockpit package metadata, flake-visible version text, provenance, and the human-facing display version mapping synchronized with `VERSION`.
