...
source archive filename:        Nix OS NAS 2.2.29 source.zip
```
...
```text
nixos-nas-2.2.0-alpha.29-source-only-unverified/
```
...
For the alpha line, increment the alpha revision (`2.2.0-alpha.28` -> `2.2.0-alpha.29`) for the next code-bearing revision. Keep `CHANGELOG.md`, Cockpit package metadata, flake-visible version text, provenance, and the human-facing display version mapping synchronized with `VERSION`.
