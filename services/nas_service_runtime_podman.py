...
application installation, drop-in merging, systemd reloads, and removal. V2
storage/network/device policy is projected as generated native Quadlet
fragments rather than re-rendering the user-authored container definition.
"""
...
from nas_managed_devices import flattened_cdi_devices, flattened_device_paths
...
GENERATED_DROPIN = "90-nixos-nas-v2-policy.conf"
...
def _v2_devices(service: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    if not isinstance(resolved, list) or any(not isinstance(item, dict) for item in resolved):
        raise ManagedServiceError("resolvedDevices must be an array of objects")
    paths = flattened_device_paths(resolved)
    cdi = flattened_cdi_devices(resolved)
    for value in [*paths, *cdi]:
        if any(char in value for char in ("\x00", "\r", "\n")):
            raise ManagedServiceError("resolved device contains an unsafe control character")
    return paths, cdi
...
def render_policy_dropin(service_id: str, service: dict[str, Any]) -> str:
    ...
    device_paths, cdi_devices = _v2_devices(service)
    if mounts or network is not None or device_paths or cdi_devices:
        ...
        for device in device_paths:
            # CDI is preferred for NVIDIA and already carries all required
            # device/library injection. Avoid duplicating raw NVIDIA nodes when
            # a CDI device is present.
            if cdi_devices and device.startswith("/dev/nvidia"):
                continue
            lines.append(f"AddDevice={device}")
        if cdi_devices:
...
def render_storage_dropin(service: dict[str, Any]) -> str:
    """Compatibility helper retained for callers that only exercise storage."""
    lines = [GENERATED_MARKER]
    mounts = _v2_mounts(service)
    if mounts:
        lines.extend(["", "[Container]"])
...
def _write_generated_policy(
    source: pathlib.Path, service_id: str, service: dict[str, Any]
) -> tuple[pathlib.Path | None, pathlib.Path | None]:
    dropin = _dropin_path(source)
    mounts = _v2_mounts(service)
    network = _v2_network(service_id, service)
    device_paths, cdi_devices = _v2_devices(service)
    has_policy = bool(mounts or network is not None or device_paths or cdi_devices)
    if not has_policy:
        try:
    ...
    return (dropin if has_policy else None, network_path)
