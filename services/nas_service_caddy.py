...
def _render_auth(lines: list[str], route: dict[str, Any], indent: str) -> None:
    ...
    lines.extend(
        [
            ...
            f"{indent}  copy_headers {{",
            f"{indent}    X-Authentik-Username>Remote-User",
            f"{indent}    X-Authentik-Groups>Remote-Groups",
            f"{indent}    X-Authentik-Name>Remote-Name",
            f"{indent}    X-Authentik-Email>Remote-Email",
            f"{indent}    X-Authentik-Uid>Remote-UID",
            f"{indent}  }}",
            f"{indent}}}",
            f"{indent}@missingAuthentikIdentity not header Remote-User *",
