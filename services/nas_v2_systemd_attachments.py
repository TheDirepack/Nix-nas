#!/usr/bin/env python3
"""Systemd-native storage, credential, and accelerator attachment policy."""
...
def _bind_value(source: pathlib.PurePosixPath, destination: pathlib.PurePosixPath) -> str:
    return str(source) if source == destination else f"{source}:{destination}"
...
def attachment_lines(effective: dict[str, Any], service: dict[str, Any]) -> list[str]:
    ...
    return [*_storage_lines(effective, service), *_credential_lines(effective, service), *_accelerator_lines(service)]
