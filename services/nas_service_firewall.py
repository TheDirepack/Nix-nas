...
def plan_firewall(service_id: str, service: dict[str, Any]) -> dict[str, Any]:
    ...
    for egress in network.get("allowedEgress", []):
        ...
        _validate_cidr(cidr)
        actions.append({"type": "allow-egress", "service": service_id, "cidr": cidr, "ports": egress.get("ports", [])})
...
def apply_firewall(service_id: str, service: dict[str, Any], *, dry_run: bool = False) -> dict[str, Any]:
    ...
    for action in plan["actions"]:
        if action["type"] == "deny-lan":
            subprocess.run(["firewall-cmd", "--permanent", "--remove-rich-rule", f'rule family="ipv4" source address="192.168.0.0/16" service name="{service_id}" accept'], check=False)
