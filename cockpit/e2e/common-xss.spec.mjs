...
async function installPayload(page, payload) {
  await page.addInitScript(value => {
    ...
    const response = {
      ...
      identity: {ok: true, users: [{uid: value}], groups: [], administrators: [], shareAuthority: value},
      ...
      links: {identity: "/identity/", documentation: "/docs/", shares: "/shares/", settings: value, files: value},
    };
    globalThis.cockpit = {
      spawn() {
...
for (const [name, payload] of payloads) {
  test(`common XSS probe is rendered inert: ${name}`, async ({page}) => {
    ...
    expect(await page.locator("script, iframe, svg, img").evaluateAll(nodes => nodes.some(node => node.outerHTML.includes("__nas_xss")))).toBe(false);
