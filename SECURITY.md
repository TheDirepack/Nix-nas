...
The standalone `/setup/` application is never anonymous. Before permanent secrets exist, an installation-unique disposable bootstrap KDBX holds only the temporary Authentik secret key, bootstrap token, and the intentionally preconfigured `akadmin` password `nas-admin-first-boot`. The human password is universal by design so a new appliance always has a known web-setup credential; it is scoped to the disposable bootstrap Authentik authority and is retired with that authority after setup. The temporary Linux `nas-bootstrap` principal remains locked, nologin, and has no password authentication.
...
After setup, Caddy + Authentik enforce request-time application capability checks. Cockpit's shared `--local-session` bridge is isolated in its own systemd network namespace; its loopback TCP listener is not reachable from the host namespace. Caddy reaches Cockpit only through a root-owned Unix socket whose socket group is `caddy`, and the default Cockpit socket/service remain disabled.
