...
class CiSummaryTests(unittest.TestCase):
    ...
    def test_pull_request_runs_staged_prebuild_prepare_and_browser(self) -> None:
        expected = ci_summary.expected_jobs("pull_request", "refs/pull/40/merge", "main", "fast")
    ...
    def test_non_main_pull_request_skips_main_coverage_baseline(self) -> None:
        expected = ci_summary.expected_jobs("pull_request", "refs/pull/40/merge", "release", "fast")
    ...
    def test_full_dispatch_includes_integration_and_installer_only(self) -> None:
        expected = ci_summary.expected_jobs("workflow_dispatch", "refs/heads/main", "", "full")
        self.assertEqual(expected, {"prebuild", "prepare", "browser", "integration", "installer"})
