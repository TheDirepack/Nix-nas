...
class ContractTests(unittest.TestCase):
    ...
    def test_ci_runs_tests_and_pins_actions(self):
        ...
        uses = [
            item for source in sources for item in re.findall(r"^\s*-?\s*uses:\s*([^\s#]+)", source, flags=re.MULTILINE)
        ]
    ...
    def test_first_run_setup_is_packaged_secure_and_vm_exercised(self):
        ...
        self.assertIn("run_as_nasadmin nas-setup account disable", guest)
        self.assertIn("alice-updated-password", guest)
        self.assertIn("first-start setup API rejects a stale plan digest", encrypted_guest)
    ...
    def test_qemu_harness_covers_native_and_installed_paths(self):
        ...
        self.assertIn("installed-security-evidence", workflow)
        ...
        self.assertIn("nas-alert", guest)
        self.assertIn("Observability and notifications", guest)
        ...
        bundle_import = handoff.index("Import restored bundle fragments")
