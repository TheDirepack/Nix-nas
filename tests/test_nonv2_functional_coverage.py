...
class NonV2FunctionalCoverageTests(unittest.TestCase):
    ...
    def test_split_groups_total_and_control_safe(self) -> None:
        ...
        self.assertEqual(set(), common.split_groups("a," * 300))
    ...
    def test_operation_lock_reuses_inherited_coordination_for_nested_work(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            op_root = pathlib.Path(tmp) / "ops"
            op_root.mkdir(mode=0o2770)
            os.chmod(op_root, 0o2770)
            with (
                mock.patch.object(oplock, "OPERATION_ROOT", op_root),
                mock.patch.dict(os.environ, {"NAS_STATE_ALLOW_UNPRIVILEGED": "1"}),
            ):
                with oplock.acquire_operation("outer", ("storage",)) as active:
                    self.assertEqual("outer", active.action)
                    self.assertIn("storage", active.classes)
                    with oplock.acquire_operation("nested", ("storage",), blocking=False) as nested:
                        self.assertIn("storage", nested.classes)

    def test_operation_reservation_is_non_binding_worker_admission_token(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            op_root = pathlib.Path(tmp) / "ops"
            op_root.mkdir(mode=0o2770)
            os.chmod(op_root, 0o2770)
            with (
                ...
                reservation = oplock.reserve_operation("async", ("network",), ttl_seconds=30)
