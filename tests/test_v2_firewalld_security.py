...
class FirewalldSecurityTests(unittest.TestCase):
    ...
    def test_rich_rule_rejects_family_mismatch(self) -> None:
        rule = ET.fromstring('<rule family="ipv4"><destination address="2001:db8::/32"/><accept/></rule>')
    ...
    def test_rich_rule_rejects_invalid_protocol_and_priority(self) -> None:
        invalid_protocol = ET.fromstring('<rule family="ipv4"><port port="443" protocol="tcp accept"/><accept/></rule>')
    ...
    def test_projection_rejects_symlink_payload(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(firewall.FirewalldReconcileError, "missing or unsafe"):
