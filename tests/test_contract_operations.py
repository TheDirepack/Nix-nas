...
class ContractTests(unittest.TestCase):
    def test_zfs_replication_and_boot_recovery_roles_are_separate(self):
        ...
        self.assertIn("shares.db", backup_resources)
        self.assertIn("sessions.db", backup_resources)
        self.assertIn("source_db.backup(destination_db)", backup_resources)
        self.assertIn('sqlite3.connect(f"file:{source}?mode=ro", uri=True)', backup_resources)
        self.assertNotIn(".backup '$destination'", backup_resources)
        self.assertIn("copyparty-database-dump", backup_resources)
        self.assertIn("native-dump", backup_resources)
        self.assertIn("./config/managed-services-backup-resources.nix", default_module)
        self.assertNotIn("source_db.backup(destination_db)", storage)
        self.assertNotIn("backup-vaultwarden.service", storage)
    ...
    def test_victoriametrics_replaces_prometheus_server(self):
        ...
        self.assertIn('routes.web = pathRoute [ "/victoriametrics/" ]', migration)
        self.assertIn("redir /victoriametrics /victoriametrics/vmui", proxy)
