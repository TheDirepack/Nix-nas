...
class OperationLockCoverageTests(unittest.TestCase):
    def test_json_views_and_empty_class_validation(self) -> None:
        active = operation_lock.ActiveOperation(
            "backup", ("storage",), 42, 100, "boot", "start", "a" * 32
        )
        self.assertEqual(
            active.as_json(),
            {
        ...
        reservation = operation_lock.OperationReservation(
            "queued", ("runtime",), "b" * 32, 10, 20, 1000, 2000
        )
    ...
    def test_conflicting_reservation_honors_ignore_token(self) -> None:
        first = operation_lock.OperationReservation(
            "one", ("runtime",), "c" * 32, 1, 2, 1, 2
        )
        second = operation_lock.OperationReservation(
            "two", ("storage",), "d" * 32, 1, 2, 1, 2
        )
        with mock.patch.object(operation_lock, "_reservations", return_value=[first, second]):
            self.assertIs(
                operation_lock._conflicting_reservation(("runtime",)), first
            )
            self.assertIsNone(
                operation_lock._conflicting_reservation(
                    ("runtime",), ignore_token=first.token
                )
            )
            self.assertIs(
                operation_lock._conflicting_reservation(
                    ("storage",), ignore_token=first.token
                ),
                second,
            )
