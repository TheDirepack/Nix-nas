...
class Alpha20CockpitContracts(unittest.TestCase):
    ...
    def test_starter_kit_build_and_nix_packaging_require_a_complete_bundle(
        self,
    ) -> None:
    ...
    def test_password_transport_and_destructive_confirmation_remain_explicit(
        self,
    ) -> None:
    ...
    def test_cache_policy_keeps_dependency_and_vm_reuse_without_pass_caching(
        self,
    ) -> None:
        workflow = text(".github/workflows/ci.yml")
        handoff = text(".github/actions/prepare-vm-handoff/action.yml")
    ...
    def test_http_adversarial_checks_use_curl_inside_installed_vm_workload(
        self,
    ) -> None:
    ...
    def test_browser_security_keeps_real_browser_engines_for_browser_semantics(
        self,
    ) -> None:
        config = text("cockpit/e2e/playwright.config.mjs")
