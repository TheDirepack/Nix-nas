...
class PasswordSecurityTests(unittest.TestCase):
    ...
    def test_setup_uses_mature_strength_and_isolated_breach_lookup(self) -> None:
        ...
        self.assertIn('breachSocket = "/run/nas-password-breach/check.sock";', source)
        self.assertIn("DynamicUser = true", source)
        self.assertIn('RestrictAddressFamilies = [ "AF_UNIX" "AF_INET" "AF_INET6" ];', source)
        self.assertIn("breach_status, breach_count = breach_check(password)", source)
        self.assertIn('return "unavailable", None', source)

    def test_privileged_setup_api_remains_network_isolated(self) -> None:
        password = PASSWORD_MODULE.read_text(encoding="utf-8")
        caddy = (ROOT / "modules/nas/config/caddy-bootstrap.nix").read_text(encoding="utf-8")
        self.assertIn('requires = [ "nas-password-breach.service" ];', password)
        self.assertIn('RestrictAddressFamilies = [ "AF_UNIX" ];', caddy)
