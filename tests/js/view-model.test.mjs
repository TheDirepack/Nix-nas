...
test("feature and link models follow effective runtime state", () => {
  ...
  assert.equal(visibleOperations(data).some(([id]) => id === "syncthing-reconcile"), true);
  assert.equal(visibleOperations(data).some(([id]) => id === "backup"), false);
});

test("service, feature runtime, and memory formatting are deterministic", () => {
  assert.equal(inactiveServiceCount([
  ...
  assert.match(featureRuntimeText({effectiveMode: "on-demand", running: false}), /authorized access/);
  assert.equal(mib(10 * 1048576), "10.0");
  assert.equal(mib(Number.NaN), "—");
});

test("setup and revision models retain recovery distinctions", () => {
  assert.deepEqual(setupModel({setup: {firstStart: {
    status: "ready",
    message: "ready",
  ...
  assert.deepEqual(revisionModel({ok: false, error: "unsafe tree"}), {kind: "error", error: "unsafe tree"});
});
...
test("backend link destinations remain same-origin root-relative paths", () => {
  ...
  for (const value of [null, 1, "", "//evil.invalid/", "https://evil.invalid/", "javascript:alert(1)", "/ok\r\nX-Test: bad"]) {
