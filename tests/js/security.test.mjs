...
test("browser sources avoid direct HTML/code injection sinks", () => {
  const sources = ["cockpit/src/app.jsx", "cockpit/src/api.js", "cockpit/src/view-model.js", "web/portal/index.html"]
    .map(text).join("\n");
  for (const sink of ["dangerouslySetInnerHTML", "document.write(", "insertAdjacentHTML(", "eval(", "new Function(", "javascript:"]) {
...
test("external-target links keep opener isolation", () => {
  ...
  const externalAnchors = [...app.matchAll(/<Button[^>]+component="a"[^>]+target="_blank"[^>]*>/g)].map(m => m[0]);
...
function fuzzValue(random, depth = 0) {
  const hostile = [
    null, true, false, 0, -1, 2 ** 31 - 1, "", "../", "' OR 1=1 --",
    '<img src=x onerror="globalThis.__nas_xss=1">', "\r\nX-Injected: yes", "A".repeat(2048),
  ];
  if (depth >= 3 || random() < 0.55) return hostile[Math.floor(random() * hostile.length)];
  if (random() < 0.5) return Array.from({length: Math.floor(random() * 6)}, () => fuzzValue(random, depth + 1));
  const result = {};
  for (let i = 0; i < Math.floor(random() * 6); i += 1) result[`k${Math.floor(random() * 20)}`] = fuzzValue(random, depth + 1);
  return result;
}
...
test("JSON output parser accepts JSON only and never evaluates injected code", () => {
  ...
  const payload = JSON.stringify({value: '<script>globalThis.__nas_xss=1</script>', nested: {constructor: {prototype: {polluted: true}}}});
  ...
  for (const invalid of ["", "{", "undefined", "(()=>{globalThis.__nas_xss=1})()", "<script>alert(1)</script>"]) {
