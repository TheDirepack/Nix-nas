...
class ManagedServicesV2CaddyTests(unittest.TestCase):
    ...
    def test_bootstrap_reconciles_portal_for_embedded_outpost(self):
        ...
        services = (ROOT / "modules/nas/config/systemd-services.nix").read_text(encoding="utf-8")
        hardening = (ROOT / "modules/nas/config/security-hardening-overrides.nix").read_text(encoding="utf-8")
        defaults = (ROOT / "modules/nas/default.nix").read_text(encoding="utf-8")
        applications = (ROOT / "modules/nas/config/application-services.nix").read_text(encoding="utf-8")
        ...
        self.assertIn("systemd.services.nas-identity-bootstrap = {", hardening)
        self.assertIn("serviceConfig.ExecStartPost = lib.mkForce [ ];", hardening)
        self.assertLess(defaults.index("./config/systemd-services.nix"), defaults.index("./config/security-hardening-overrides.nix"))
