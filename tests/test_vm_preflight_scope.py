...
class VmPreflightScopeTests(unittest.TestCase):
    ...
    def test_full_stack_vm_profiles_existing_phase_boundaries(self) -> None:
        ...
        self.assertIn("VM-PHASE-START:", vm_common)
        self.assertIn("VM-PHASE-TIMING:", vm_common)
        self.assertIn("VM-FIRST-RUN-START:", vm_common)
        self.assertIn("VM-FIRST-RUN-TIMING:", vm_common)
        self.assertIn("VM-PHASE-BUDGET:", vm_common)
        self.assertIn("trap 'nas_vm_profile_command \"$BASH_COMMAND\"' DEBUG", vm_common)
        self.assertIn("nas_vm_cleanup_add nas_vm_profile_cleanup", vm_common)
        self.assertIn('run_as_admin*"nas-setup first-run"*', vm_common)
